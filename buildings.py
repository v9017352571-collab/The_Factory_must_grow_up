...
RESOURCES_COST = {
    'Угольный бур': {'Медь': 10, 'Олово': 6, 'Уголь': 0, 'Бронза': 0, 'Кремний': 0, 'Боеприпасы': 0},
    ...
    'Дроны': {'Медь': 5, 'Олово': 3, 'Уголь': 0, 'Бронза': 0, 'Кремний': 1, 'Боеприпасы': 0},
    'Ядро': {}
}
...
Завод Боеприпасыов - 8 Медь, 15 Олово, Уголь, Бронза, 24 Кремний, бп (потребляет: Уголь, Олово и Кремний, выдает: бп)
...
class BronzeFurnace(ProductionBuilding):
    ...
    def __init__(self, x: float, y: float):
        ...
        self.hp = BUILDING_HP["Бронзовая плавильня"]
        self.max_hp = self.hp
...
class SiliconFurnace(ProductionBuilding):
    ...
    def __init__(self, x: float, y: float):
        super().__init__(
            image_path="Изображения/Здания/Заводы/Печь_кремния.png",
            scale=0.25,
            x=x, y=y,
            production_time=2.0,
            name="Кремнева плавильня"
        )
        self.hp = BUILDING_HP["Кремнева плавильня"]
        self.max_hp = self.hp
